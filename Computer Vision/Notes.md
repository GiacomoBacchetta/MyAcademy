...
---
The first kind of loss function we talk about is the multi-class SVM loss function.
...
After we have a loss for the whole training dataset which is the average of each SVM loss calculated

$L = \frac{1}{N} \sum_{i=1}^{N}{L_i}$ 

Going on, we can introduce the regularization, wihch is a technique to manage an incremental complexity of the model we are using. We know several type of regularization terms:
- L1 regularization;
- L2 regularization;
- Elastic net (L1 + L2)
- Max Norm Regularization
This terms measure the complexity of the model and are aded to the loss function which has to been minimize.


Other tecniques which have the same objective of the regularization are the dropout and the batch normalization.

Obviously, there exist very different loss functions, and they depend often by the algorithm we implement. In the classification problem, we can also implement a Softmax Classifier, an algortihm which has a probability vector as ouput. 
